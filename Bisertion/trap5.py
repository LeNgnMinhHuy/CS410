...
for i in l:
    ...
    MRPS, average_number_of_evaluations = bisection(i, MSSV, use_onemax=False)
    if len(MRPS) == 0:
        print(f"N upper > 8192, Stop at l = {i}")
        break
    Trap_1X_result['MRPS'].append(MRPS)
    Trap_1X_result['average_number_of_evaluations'].append(average_number_of_evaluations)

save_path = "Trap_1X_result.py"
with open(save_path, "w+") as f:
    f.writelines(f"result={Trap_1X_result}")
...
for i in l:
    ...
    print("--------------------------------------------------------")
    print(f"Problem size = {i}")
    MRPS, average_number_of_evaluations = bisection(i, MSSV, use_onemax=False, use_1X=False)
    if len(MRPS) == 0:
        print(f"N upper > 8192, Stop at l = {i}")
        break
    Trap_UX_result['MRPS'].append(MRPS)
    Trap_UX_result['average_number_of_evaluations'].append(average_number_of_evaluations)

save_path = "Trap_UX_result.py"
with open(save_path, "w+") as f:
    f.writelines(f"result={Trap_UX_result}")
